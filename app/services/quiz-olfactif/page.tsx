...
/* ─── Résultats simulés (en production : API ou algo) ────── */

const recommendations: Record<string, { nom: string; maison: string; famille: string; description: string; slug: string }[]> = {
  default: [
    {
      nom: 'Sauvage Elixir',
      maison: 'Dior',
      famille: 'Boisé-Oriental',
      description: "Un concentré intense de lavande et de poivre, sur un fond d'encens fumé. Signature masculine absolue.",
      slug: '/produits?q=Sauvage%20Elixir',
    },
    {
      nom: 'Black Opium',
      maison: 'Yves Saint Laurent',
      famille: 'Floral-Gourmand',
      description: "Café noir, jasmin blanc, vanille crémeuse. Séduisant et addictif, parfait pour les soirées.",
      slug: '/produits?q=Black%20Opium',
    },
    {
      nom: 'Baccarat Rouge 540',
      maison: 'Maison Francis Kurkdjian',
      famille: 'Floral-Ambré',
      description: "Jasmin, safran, ambre gris et bois de cèdre. Un olfactif qui dure des heures et marque les esprits.",
      slug: '/produits?q=Baccarat%20Rouge%20540',
    },
    {
      nom: 'Good Girl',
      maison: 'Carolina Herrera',
      famille: 'Oriental-Floral',
      description: "Jasmin, tonka, cacao. Un parfum dualiste — lumière et ombre — dans un flacon stiletto iconic.",
      slug: '/produits?q=Good%20Girl',
    },
    {
      nom: 'Bleu de Chanel',
      maison: 'Chanel',
      famille: 'Boisé-Aromatique',
      description: "Citron, gingembre, santal blanc. Une élégance fraîche et contemporaine aux notes profondes.",
      slug: '/produits?q=Bleu%20de%20Chanel',
    },
  ],
};
...
export default function QuizOlfactifPage() {
  ...
  const reco = recommendations.default;
        ...
        <section style={{ background: 'var(--surface)', padding: '72px 0' }}>
          <div className="container">
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 2 }}>
              {reco.map((r, i) => (
                <div key={r.nom} style={{ background: '#fff', padding: '32px 28px', borderTop: '2px solid var(--line-light)', position: 'relative' }}>
                  ...
                  <h3 style={{ fontFamily: 'var(--font-serif)', fontSize: '1.375rem', fontWeight: 400, color: 'var(--text-primary)', margin: '8px 0 2px', lineHeight: 1.2 }}>{r.nom}</h3>
                  <p style={{ fontSize: '0.75rem', letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--text-pale)', margin: '0 0 6px' }}>{r.maison} · {r.famille}</p>
                  <div style={{ width: 24, height: '1px', background: 'var(--gold)', margin: '12px 0', opacity: 0.5 }} aria-hidden="true" />
                  <p style={{ fontSize: '0.8125rem', color: 'var(--text-secondary)', lineHeight: 1.7, margin: '0 0 20px' }}>{r.description}</p>
                  <Link href={r.slug} style={{ fontSize: '0.6875rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--gold)', textDecoration: 'none', fontWeight: 600, borderBottom: '1px solid rgba(197,165,90,0.35)', paddingBottom: 3 }}>
                    Voir le produit →
                  </Link>
                </div>
              ))}
