...
const testimonials = [
  { texte: "La consultation a complètement changé ma relation aux parfums. Je suis passée de perdue à confiante.", nom: "Fatoumata D.", ville: "Abidjan" },
  { texte: "En 90 minutes, l'experte a cerné exactement ce que je cherchais depuis des années. Impressionnant.", nom: "Marie-Claire A.", ville: "Ouagadougou" },
];
      ...
      <section style={{ background: 'var(--offwhite)', padding: '72px 0', borderTop: '1px solid var(--line-light)' }}>
        <div className="container">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2 }} className="testi-grid">
            {testimonials.map((t) => (
              <div key={t.nom} style={{ background: '#fff', padding: '36px 32px', borderLeft: '3px solid var(--gold)' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'var(--gold-muted)', border: '1px solid rgba(197,165,90,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <span style={{ fontFamily: 'var(--font-serif)', fontSize: '0.875rem', color: 'var(--gold)', fontWeight: 500 }}>{t.nom[0]}</span>
                  </div>
                  <div>
                    <p style={{ fontSize: '0.8125rem', fontWeight: 600, color: 'var(--text-primary)', margin: 0 }}>{t.nom}</p>
