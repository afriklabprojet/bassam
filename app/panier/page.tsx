...
export default function CartPage() {
  ...
  const VALID_CODES: Record<string, number> = { VIP10: 0.1, BIENVENUE: 0.15 };
  const discount = promoApplied ? VALID_CODES[promoCode.toUpperCase()] ?? 0 : 0;
              ...
              <div className="card" style={{ padding: '1.5rem' }}>
                ...
                {promoApplied ? (
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.875rem', fontWeight: 500, color: 'var(--text-primary)' }}>
                    ...
                    <span>Code <strong style={{ fontFamily: 'var(--font-serif)' }}>{promoCode.toUpperCase()}</strong> — {Math.round(discount * 100)}% de réduction</span>
                  </div>
                ) : (
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                    ...
                    />
                    <button onClick={applyPromo} className="btn-ghost" style={{ flexShrink: 0 }}>
                      Appliquer
                    </button>
                  </div>
                )}
