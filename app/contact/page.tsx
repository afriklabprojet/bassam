...
import { buildWhatsAppHref, getSupportPhoneHref, hasWhatsAppSupport, supportConfig } from '@/lib/site-config';
...
function buildCanaux() {
  const canaux = [];
  const phoneHref = getSupportPhoneHref();
  const whatsappValue = supportConfig.whatsappDisplay || (supportConfig.whatsappNumber ? `+${supportConfig.whatsappNumber}` : '');
  const phoneValue = supportConfig.phoneDisplay || (supportConfig.supportPhone ? `+${supportConfig.supportPhone}` : '');
  ...
  if (hasWhatsAppSupport()) {
    canaux.push({
      ...
      href: buildWhatsAppHref('Bonjour VIP Parfumerie Bar, je souhaite vous contacter.'),
  ...
  canaux.push({
    ...
    sous: 'Livraison dans toute l’Afrique de l’Ouest',
    href: undefined,
    label: undefined,
    extern: false,
  });
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '72px', alignItems: 'start' }} className="form-grid">
            ...
            <div>
              ...
              <p style={{ fontSize: '0.875rem', color: 'var(--text-secondary)', lineHeight: 1.75, margin: '0 0 40px' }}>
                ...
                {hasWhatsAppSupport() ? ' Pour une réponse immédiate, privilégiez WhatsApp.' : ' Pour une réponse rapide, privilégiez l’email.'}
              </p>
              ...
              <div style={{ marginTop: 40 }}>
                ...
                <p style={{ fontSize: '0.875rem', color: 'var(--text-secondary)', margin: 0 }}>
                  ...
                  <span style={{ color: 'var(--gold)', fontWeight: 500 }}>{hasWhatsAppSupport() ? 'WhatsApp disponible 7j/7' : 'Réponse email sous 24h ouvrées'}</span>
                </p>
              </div>
            </div>
