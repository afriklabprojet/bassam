...
export const CARD_HOVER = 'bento-card';
  ...
  return (
    <div className={`${CARD_HOVER} lg:col-span-8 relative`} style={{ ...CARD_BASE, background: 'linear-gradient(135deg, rgba(197,165,90,0.06) 0%, rgba(255,255,255,0.015) 50%, rgba(197,165,90,0.03) 100%)', border: '1px solid rgba(197,165,90,0.1)', padding: 0 }}>
      ...
      <div className="relative z-10 p-8 pb-0">
        ...
        <div className="stat-value" style={{ fontSize: 'clamp(2rem, 5vw, 3.25rem)', fontWeight: 700, fontFamily: 'var(--font-serif)', lineHeight: 1.1, marginTop: '8px' }}>
...
export function QuickActionsCard() {
  return (
    <div className={`${CARD_HOVER} lg:col-span-4 flex flex-col`} style={{ ...CARD_BASE, padding: '28px' }}>
      ...
      <div className="flex flex-col gap-3 flex-1">
        {QUICK_ACTIONS.map((action) => (
          <Link key={action.label} href={action.href} className="flex items-center gap-3.5 px-4 py-3.5 rounded-xl transition-all duration-200 group/action" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(197,165,90,0.08)' }}>
            ...
            <svg className="ml-auto opacity-0 group-hover/action:opacity-100 transition-all duration-200 group-hover/action:translate-x-0.5" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#555" strokeWidth={2}>
...
export function OrdersTable({ orders }: Readonly<{ orders: RecentOrder[] }>) {
  return (
      ...
      {orders.length === 0 ? (
        <div className="px-7 py-20 text-center">
          <div className="w-14 h-14 rounded-2xl flex items-center justify-center mx-auto mb-4" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#333" strokeWidth={1.5}><path d="M6 2L3 6v14a2 2 0 002 2h14a2 2 0 002-2V6l-3-4z" /><line x1="3" y1="6" x2="21" y2="6" /><path d="M16 10a4 4 0 01-8 0" /></svg>
          </div>
            ...
            <thead>
              <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                {['Réf.', 'Client', 'Montant', 'Statut', 'Date'].map((h) => (
                  <th key={h} className="px-7 py-3.5 text-left" style={{ color: '#444', fontWeight: 600, fontSize: '0.6875rem', textTransform: 'uppercase', letterSpacing: '0.08em' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {orders.map((order, i) => {
                ...
                return (
                  <tr key={order.id} className="transition-colors duration-150 hover:bg-white/1.5" style={{ borderBottom: i < orders.length - 1 ? '1px solid rgba(255,255,255,0.03)' : 'none' }}>
                    ...
                    <td className="px-7 py-4" style={{ color: '#555', fontSize: '0.75rem' }}>{formatDate(order.createdAt)}</td>
                  </tr>
                );
              })}
            </tbody>
...
export function StatusBreakdown({ orders, totalOrders }: Readonly<{ orders: RecentOrder[]; totalOrders: number }>) {
  return (
    <div className={`${CARD_HOVER} lg:col-span-4 flex flex-col`} style={{ ...CARD_BASE, padding: '28px' }}>
      <span style={{ fontSize: '0.6875rem', fontWeight: 600, color: '#555', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '24px' }}>Répartition statuts</span>
...
export function TopProductsTable({ products }: Readonly<{ products: TopProduct[] }>) {
  return (
    <div className={`${CARD_HOVER} lg:col-span-8`} style={{ ...CARD_BASE, padding: 0 }}>
      <div className="flex items-center justify-between px-7 py-5" style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
        <div>
...
export function PaymentMethodsCard({ paymentStats }: Readonly<{ paymentStats: PaymentStat[] }>) {
  return (
    <div className={`${CARD_HOVER} lg:col-span-4 flex flex-col`} style={{ ...CARD_BASE, padding: '28px' }}>
      <span style={{ fontSize: '0.6875rem', fontWeight: 600, color: '#555', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '24px' }}>Méthodes de paiement</span>
      {paymentStats.length === 0 ? (
        <div className="flex-1 flex items-center justify-center"><p style={{ color: '#444', fontSize: '0.8125rem' }}>Aucune donnée</p></div>
      ) : (
              ...
              <div key={pm.method}>
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2.5">
                    ...
                    <span style={{ fontSize: '0.8125rem', color: '#aaa' }}>{PAYMENT_LABELS[pm.method] ?? pm.method}</span>
                  </div>
...
export function TopCustomersTable({ customers }: Readonly<{ customers: TopCustomer[] }>) {
  return (
    <div className={`${CARD_HOVER} lg:col-span-8`} style={{ ...CARD_BASE, padding: 0 }}>
      <div className="flex items-center justify-between px-7 py-5" style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
        <div>
      ...
      {customers.length === 0 ? (
        ...
        <div className="divide-y divide-white/3">
          {customers.map((customer, i) => {
            ...
            return (
              <div key={customer.email} className="flex items-center gap-4 px-7 py-4 transition-colors duration-150 hover:bg-white/1.5">
                <span style={{ fontSize: '0.6875rem', color: '#444', fontWeight: 600, width: '16px', textAlign: 'center', flexShrink: 0 }}>{i + 1}</span>
                <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0 text-xs font-bold" style={{ background: av.bg, color: av.color, border: `1px solid ${av.border}` }}>{initials}</div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="min-w-0">
...
export function LowStockCard({ products, lowStockCount }: Readonly<{ products: LowStockProduct[]; lowStockCount: number }>) {
  return (
